...
def handle_nurseries(entry_type):
    ...
    if entry_type == "Single Entry":
        ...
        if st.button("Add Nursery", key="single_entry_submit"):
            # Check if the nursery already exists
            check_query = "SELECT nursery_id FROM Nurseries WHERE nursery_name = %s;"
            existing = run_query(check_query, (nursery_name,))
            ...
            if already_uploaded:
                st.info("✅ Nursery already uploaded. Record updated successfully!")
            else:
        ...
        if file is not None:
            ...
            if st.button("Confirm Bulk Upload"):
                ...
                for _, row in df.iterrows():
                    check_query = "SELECT nursery_id FROM Nurseries WHERE nursery_name = %s;"
                    existing = run_query(check_query, (row["nursery_name"],))
                    if existing:
                        update_count += 1
                    else:
                        new_count += 1
                ...
                st.success(f"✅ Bulk nurseries processed! {new_count} new, {update_count} updated.")
