...
def search_page():
    ...
    # Add Address dropdown filter (populated from Nurseries joined with Nursery_Tree_Inventory)
    query_address = """
        SELECT DISTINCT n.address 
        FROM Nursery_Tree_Inventory nti
        JOIN Nurseries n ON nti.nursery_name = n.nursery_name;
    """
    addresses = [row["address"] for row in run_query(query_address) or []]
    selected_address = st.selectbox("Select Address", ["All"] + addresses)
    ...
    if st.button("Search Inventory", use_container_width=True):
        ...
        if selected_address != "All":
            conditions.append("n.address = %s")
            params.append(selected_address)
        ...
        if conditions:
            ...
            # Updated query: join photos to fetch photo_url from photos table instead of using main_photo_url from Trees
            query = f"""
                ...
                for row in results:
                    ...
                    <div class="result-card">
                        <div class="result-header">
                            <img src="{row['photo_url']}" alt="Tree Photo">
                            <div>
