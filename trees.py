...
def handle_trees(entry_type):
    ...
    if entry_type == "Single Entry":
        ...
        if st.button("🌳 Add Tree"):
            ...
            check_query = "SELECT * FROM Trees WHERE common_name = %s;"
            existing = run_query(check_query, (common_name,))
            if existing:
                st.info("This tree was already uploaded.")
            else:
                query = """
                INSERT INTO Trees (common_name, scientific_name, growth_rate, watering_demand, shape, care_instructions, origin, soil_type, root_type, leafl_type)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s);
                """
                execute_query(query, (common_name, scientific_name, growth_rate, watering_demand, shape, care_instructions, origin, soil_type, root_type, leafl_type))
        ...
        <p>Upload a CSV file with tree details. Ensure columns match: common_name, scientific_name, growth_rate, watering_demand, shape, care_instructions, origin, soil_type, root_type, leafl_type</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if file is not None:
            ...
            if st.button("📂 Confirm Bulk Upload"):
                ...
                for index, row in df.iterrows():
                    check_query = "SELECT * FROM Trees WHERE common_name = %s;"
                    existing = run_query(check_query, (row["common_name"],))
                    if existing:
                        skipped_count += 1
                    else:
                        query = """
                        ...
                        inserted_count += 1
                st.success(f"✅ Bulk upload completed! Inserted: {inserted_count}, Skipped (already uploaded): {skipped_count}")
        ...
        if data:
            ...
            with action_tabs[0]:
                ...
                if st.button("💾 Update Tree"):
                    ...
                    care_instructions=%s, origin=%s, soil_type=%s, root_type=%s, leafl_type=%s
                    WHERE tree_id=%s;
                    """
                    execute_query(update_query, (common_name, scientific_name, growth_rate, watering_demand, shape, care_instructions, origin, soil_type, root_type, leafl_type, selected_id))
